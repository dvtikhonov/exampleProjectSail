...
└── tests/
    ...
    ├── Unit/EloquentGatewayUserResolverTest.php
...
Middleware `TrustGatewayAuth` читает заголовок `X-User-Id`, разрешает пользователя через `GatewayUserResolverInterface` и выполняет `Guard::login` через `GatewayAuthSessionInterface`. При отсутствии или невалидном заголовке (`пустой`, нечисловой, `<= 0`) — `401` с телом `{"message": "Unauthorized gateway user."}` (`GatewayUnauthorizedResponse`).

`EloquentGatewayUserResolver`:

1. Ищет `User` по `X-User-Id` в таблице `users` (общая MySQL с `main-app`, миграции `users` — в `main-app`, не в `service-a`).
2. Если запись не найдена — создаёт stub-пользователя (`gateway-user-{id}@gateway.local`) через `firstOrCreate` (fallback для рассинхрона id; в штатном контуре после web-login пользователь уже есть в общей БД).
...
Для вызовов через gateway браузер передаёт `Authorization: Bearer <passport-token>`; gateway проверяет JWT через `main-app` `/api/auth/verify` и добавляет `X-User-Id`.
...
Типичный сценарий из `main-app`: Inertia-страница на `http://localhost` (main-app, порт `80`) шлёт cross-origin `PATCH` / `POST` / `DELETE` на `${VITE_GATEWAY_ORIGIN}/api/a/sales-outlets/...` (по умолчанию `http://localhost:8080`) с `Authorization: Bearer <token>`. Origin страницы и URL gateway **намеренно различаются** — nginx отвечает `Access-Control-Allow-Origin: $http_origin`, поэтому preflight и основной запрос проходят. Браузер сначала может отправить preflight `OPTIONS` на gateway — его закрывает nginx (`204`), затем идёт основной запрос с проверкой токена (`auth_request` → `main-app` `/api/auth/verify`) и проксированием в `service-a` с `X-User-Id`.

Если при разработке фронтенда видны ошибки CORS:

1. Убедитесь, что API вызывается через **`/api/a/`**, а не напрямую на порт `8081`.
2. Проверьте, что `VITE_GATEWAY_ORIGIN` указывает на gateway (`http://localhost:8080`), а не на origin страницы.
...
`service-a` использует **ту же MySQL**, что и `main-app` (`DB_*` в `.env`). Таблица `users` создаётся миграциями `main-app` (`0001_01_01_000000_create_users_table.php`) и общая для gateway-auth; отдельной миграции `users` в `service-a` нет.

Таблица `sales_outlets` (миграции только в `service-a`):
...
Клиентские мутации (`updateSalesOutlet`, `updateHeadOrganization`) идут **только через gateway** (см. [CORS](#cors)) — `main-app/resources/js/Services/salesOutlets.js` + `VITE_GATEWAY_ORIGIN`:

```text
GET    ${VITE_GATEWAY_ORIGIN}/api/a/sales-outlets          # SSR через SalesOutletsApiClient (сервер main-app)
PATCH  ${VITE_GATEWAY_ORIGIN}/api/a/sales-outlets/{rowId}  # браузер, authorizedJsonRequest
POST   ${VITE_GATEWAY_ORIGIN}/api/a/sales-outlets/{rowId}/head-organization
```

Токен для gateway (`main-app/resources/js/Services/apiToken.js`):

| Шаг | Поведение |
|---|---|
| После web-login | `main-app` создаёт Passport-токен в сессии (`passport_token`); старые токены пользователя отзываются |
| `GET /get-api-token` | Возвращает токен из сессии (источник истины; не читает устаревший кэш без запроса к серверу) |
| `authorizedJsonRequest` | Добавляет `Authorization: Bearer`, при `401`/`403` сбрасывает кэш и повторяет запрос с новым токеном |
| Logout | `localStorage` (`api_token`) очищается в `AuthenticatedLayout` |

Прокси-маршрутов для PATCH/POST в `main-app/routes/web.php` нет — мутации не идут через `/objects-sales-outlets/...`.

Экспорт, почта, MAX и live-статистика отчётов идут через `service-b` (прокси `main-app`), не через прямой gateway-CRUD `service-a`.
...
| `tests/Unit/EloquentGatewayUserResolverTest.php` | `X-User-Id` → DTO, provision stub-user, невалидный заголовок → `null` |
