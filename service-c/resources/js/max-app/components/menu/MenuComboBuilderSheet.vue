<script setup>
/**
 * Bottom sheet сборки комбо из двух блюд.
 */
defineProps({
    open: {
        type: Boolean,
    ...
    <Teleport to="body">
        <div v-if="open" class="fixed inset-0 z-30">
            <button
                ...
                aria-label="Закрыть сборку комбо"
                @click="$emit('close')"
            />

            <section
                class="absolute inset-x-0 bottom-0 rounded-t-[var(--radius-menu-sheet)] bg-white px-4 pb-6 pt-4 shadow-2xl safe-area-bottom"
                role="dialog"
                aria-modal="true"
                aria-labelledby="combo-builder-title"
            >
                <div class="mx-auto mb-4 h-1 w-10 rounded-full bg-gray-200" aria-hidden="true" />
