...
|---|---|
| `MAX_REPORT_CHAT_IDS` | Chat ID получателей заказов и кнопки **«тест бот»** (через запятую) |
| `MAX_REPORT_USER_IDS` | User ID получателей заказов и **«тест бот»** (через запятую) |
...
Для тестовых рассылок UI Stand и кнопки **«тест бот 2»** используйте отдельные `MAX_UI_STAND_*` — см. [UI Stand и тестовые кнопки бота](#ui-stand-и-тестовые-кнопки-бота).
...
|---|---|---|---|
| Отчёты / заказы | `MAX_REPORT_CHAT_IDS`, `MAX_REPORT_USER_IDS` | `POST /api/food/orders/submit`, кнопка **«тест бот»** в админке меню | Уведомление о заказе / `Тест БОТ` |
| UI Stand (тест) | `MAX_UI_STAND_CHAT_IDS`, `MAX_UI_STAND_USER_IDS` (+ кэш webhook) | `max:ui-stand:send`, кнопка **«тест бот 2»** в админке меню | Приветствие UI Stand / `тест бот 2` |

Пример разделения чатов (prod/dev):

```env
# Чат отчётов «Обедов» — только заказы и «тест бот»
MAX_REPORT_CHAT_IDS=434832398

# Группа Home_chat — только UI Stand и «тест бот 2»
...
> **Не путать с чатом по заказу:** внутренний чат клиент ↔ админ в mini-app идентифицируется `order_id` (`/api/food/orders/{id}/messages`), а не MAX `chat_id`.
...
| `MAX_UI_STAND_CHAT_IDS` | Chat ID для UI Stand и кнопки **«тест бот 2»** (через запятую); отдельно от `MAX_REPORT_*` |
| `MAX_UI_STAND_USER_IDS` | User ID для UI Stand и **«тест бот 2»** (личные диалоги; через запятую) |
| `MAX_REPORT_CHAT_IDS` | Chat ID для уведомлений о заказах и кнопки **«тест бот»** (те же ID, что в service-b; через запятую) |
| `MAX_REPORT_USER_IDS` | User ID для уведомлений о заказах и **«тест бот»** (те же ID, что в service-b; через запятую) |
...
2. **`MaxMenuAvailabilityNotifier::notify()`** — отправляет текст `Доступно для заказов меню на ДД.ММ.ГГГГ` в MAX:
   - чаты и пользователи из `MAX_REPORT_CHAT_IDS` / `MAX_REPORT_USER_IDS` (как у уведомлений о заказах);
...
| 8c | Войти как `menu_manager` (1005) → «Меню» → категории / график / импорт XLS/XLSX (опционально) | Категории и блюда создаются/обновляются; комбо доступно в категориях с `is_combo_available` |
