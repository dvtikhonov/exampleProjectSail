...
| [Уведомления клиенту о результате проверки](#уведомления-клиенту-о-результате-проверки) | Submitted / confirmed / rejected / состав изменён |
...
|---|---|
| `useAuth` | `POST /api/max/auth`, Bearer, `admin_roles` |
| `useCart` | Корзина, debounce адреса (500 ms), submit, бейдж по группам (`countCartGroupsQuantity`) |
| `useRestaurantsMenu` | Рестораны / меню / add / combo; **single-restaurant mode** |
| `useClientNavigation` | «Назад» / «домой» с учётом single-restaurant |
...
Реализация: `CartService`, `CartTotalsCalculator`, `CartDtoFactory`; UI — `CartPage` → `components/cart/*` (`CartHeader`, `CartItemList`, `CartSummaryFooter`, `CartOrderConfirmModal`, `CartDeliveryHint`), scope — `cartScope.js`.
...
| MAX (пакет Max) | `MaxMenuAvailabilityNotifier` (`Services/Max/UiStand/`), `LaravelMaxAdminBotTestSender` |
| Профиль | `MaxUserDeliveryAddressService` |

Контракты — `app/Contracts/Food/` (сервисы: `CartServiceInterface`, `OrderSubmissionServiceInterface`, `OrderChatServiceInterface`, `CustomerOrderQueryServiceInterface`, `DishAdminServiceInterface`, `MenuCategoryAdminServiceInterface`, `DishAvailabilityScheduleServiceInterface`, `OrderCompositionUpdateServiceInterface`, `OrderCompositionSnapshotBuilderInterface`, notifiers; репозитории: `CartRepositoryInterface`, раздельные read/write заказов `FoodOrderWriteRepositoryInterface` / `FoodOrderCustomerReadRepositoryInterface` / `FoodOrderAdminReadRepositoryInterface`, `FoodOrderAdminRepositoryInterface`, `DishAdminRepositoryInterface` / `DishCatalogRepositoryInterface` → один `EloquentDishRepository`, `DishAvailabilityRepositoryInterface`, `MenuCategoryRepositoryInterface`, `MenuReadRepositoryInterface`, `RestaurantRepositoryInterface`, `DeliveryTierRepositoryInterface`, `CustomerCategoryRepositoryInterface`, `OrderMessageRepositoryInterface`, image: `DishImageUploadInterface`, `DishImageDeliveryInterface`, `DishImageUrlResolverInterface`) и `app/Contracts/Max/` (`MaxAdminBotTestSenderInterface`, `MaxMenuAvailabilityNotifierInterface`, `MaxUserRepositoryInterface`, `MaxWebAppInitDataValidatorInterface`, `MaxWebhookUpdateRouterInterface`, `MaxOrderNotificationConfigProviderInterface`). Shared: `Shared\MaxMessenger\Contracts\MaxBotTokenProviderInterface` → `EnvMaxBotTokenProvider`. Eloquent-реализации — `app/Repositories/Food/`, `app/Repositories/Max/`. Привязки DI — `AppServiceProvider`. Ошибки домена — `FoodDomainException` → JSON `{ message }` с HTTP 4xx.
...
|---|---|---|
| Принят на рассмотрение | Сразу после `POST /orders/submit` (после commit) | `OrderSubmissionService` → `LaravelFoodOrderCustomerNotifier::notifySubmitted` (+ кнопка «Открыть заказ №N») |
...
| Отклонение | Любой этап → `rejected` | `OrderReviewStepHandler::reject` → `LaravelFoodOrderCustomerNotifier::notifyRejected` (scope из `OrderReviewStep::rejectionScope`) |
| Состав изменён | После успешного `PUT .../composition` | `OrderCompositionUpdateService` → `LaravelFoodOrderCustomerNotifier::notifyCompositionChanged` (+ кнопка «Открыть заказ №N») |

Сборка текста — `FoodOrderMaxMessageBuilder` (`buildCustomerSubmitted`, `buildCustomerConfirmed`, `buildCustomerRejected`, `buildCustomerCompositionChanged`). Сбой MAX **не откатывает** заказ / решение / правку состава; ошибка логируется в `messMax`.
...
│   │   ├── Food/                   # Cart*, Dish* (admin/catalog/availability/image), FoodOrder* (read/write/admin),
│   │   │                           # Menu*, Restaurant*, DeliveryTier*, CustomerCategory*, OrderChat*,
│   │   │                           # OrderSubmission*, CustomerOrderQuery*, OrderComposition*, notifiers
...
│   │                               # FoodOrder, FoodOrderMessage, FoodOrderChatRead, FoodOrderAdmin,
│   │                               # MaxUser, CustomerCategory, RestaurantCategoryDeliveryTier, User
│   ├── Repositories/
...
│   │   │                           # useCompositionEdit, useOrderDetailPaneLayout,
...
│   │   │                           # AdminDishList/FormPage, AdminDishAvailabilityPage,
│   │   │                           # AdminMenuCategoryList/FormPage,
...
В MAX mini-app: `address_reviewer` — раздел «Заказы» (адрес + оплата); `composition_reviewer` — очередь по составу с возможностью **редактировать состав** до approve/reject; при нескольких ролях — переключатель «Заказы» / «Меню».
...
| MAX-уведомления (клиент / статус) | `tests/Unit/FoodOrderCustomerNotifierTest.php` (`notifySubmitted`, confirmed/rejected, `notifyCompositionChanged`) |
