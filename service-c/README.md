...
Backend (Laravel 13, PHP 8.4) и Vue 3 SPA для MAX mini-app: сеть ресторанов → меню → корзина → заявка → проверка администраторами (адрес, оплата, состав) → подтверждение или отклонение. Клиент видит «Мои заказы», чат по заказу и уведомления в MAX. Расчёт доставки по категории клиента и порогам суммы заказа. Админ меню: CRUD блюд и импорт из XLS/XLSX. Также webhook MAX, UI Stand (приветствие + inline-кнопки), локальная отладка в браузере (`MAX_LOCAL_DEV_INIT_DATA`) и Artisan-команды `max:bot:info`, `max:webhook:*`, `max:ui-stand:send`, `max:food-admin:assign`.
...
| Админ меню | `menu_manager` | CRUD блюд, импорт из XLS/XLSX |
...
Реализация: `CartDeliveryAddressService`, `MaxUserDeliveryAddressService`.
...
2. Строится `items_snapshot` (название, цена, количество, `image_url`).
...
| Заказ | `OrderSubmissionService`, `CustomerOrderQueryService`, `AdminOrderQueryService` |
...
| Меню (админ) | `DishAdminService`, `DishSpreadsheetImportService`, `DishImageUploadService`, `DishImageDeliveryService`, `DishImageUrlResolver` |
...
Контракты репозиториев — `app/Contracts/Food/`; Eloquent-реализации — `app/Repositories/Food/`. Ошибки домена — `FoodDomainException` → JSON `{ message }` с HTTP 4xx.
...
│   ├── DTO/                        # Food (Cart, Order, Admin*, OrderMessage, ImportDishRowDto,
│   │                               # DishImportResultDto), Max, Auth
...
│   │   │                           # DishAdmin*, DishSpreadsheetImport*, DishSpreadsheetRowParser,
...
│   │   └── Max/                    # initData validator, mini-app auth, MaxUserDeliveryAddressService,
│   │                               # UiStand/*, ConfigMaxOrderNotificationConfigProvider
...
│   │                               # RejectOrderModal (импорт XLS/XLSX на AdminDishList)
...
В MAX mini-app пользователь с ролью `menu_manager` попадает в раздел «Меню» (CRUD блюд, кнопка «Загрузить» для импорта). Если есть и роли проверки заказов, и `menu_manager` — переключатель «Заказы» / «Меню».
...
| Admin dishes CRUD / import | `tests/Feature/AdminDishApiTest.php` |
