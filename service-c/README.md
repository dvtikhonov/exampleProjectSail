...
Backend (Laravel 13, PHP 8.4) и Vue 3 SPA для MAX mini-app: сеть ресторанов → меню → корзина → заявка → проверка администраторами (адрес, оплата, состав) → подтверждение или отклонение. Клиент видит «Мои заказы», чат по заказу и уведомления в MAX. Расчёт доставки по категории клиента и порогам суммы заказа. Также webhook MAX, UI Stand (приветствие + inline-кнопки), локальная отладка в браузере (`MAX_LOCAL_DEV_INIT_DATA`) и Artisan-команды `max:bot:info`, `max:webhook:*`, `max:ui-stand:send`, `max:food-admin:assign`.
...
- **Корзина** — одна черновая (`draft`) на пользователя MAX; позиции только из одного ресторана (иначе `422` с сообщением о несовпадении ресторана).
...
- **Оформление заявки** (`POST /api/food/orders/submit`) — создаёт заказ со статусом `pending_review` и тремя этапами проверки (`address`, `payment`, `composition`) в статусе `pending`. После commit — уведомление в MAX-чаты отчётов (см. [Уведомления о заказах в MAX](#уведомления-о-заказах-в-max)).
- **Проверка заказа** — три независимых этапа (`address_review_status`, `payment_review_status`, `composition_review_status`). Итоговый `status` вычисляет `OrderStatusResolver`: при любом `rejected` → `rejected`; при всех `approved` → `confirmed`; иначе → `pending_review`. Адрес и **оплату** проверяет `address_reviewer`, состав — `composition_reviewer`. При полном подтверждении или отклонении клиент получает личное сообщение в MAX (см. [Уведомления клиенту](#уведомления-клиенту-о-результате-проверки)).
- **Мои заказы** — клиент видит список и детали своих заказов (`GET /api/food/orders`, `GET /api/food/orders/{id}`).
- **Чат по заказу** — клиент и администраторы обмениваются сообщениями (`GET`/`POST /api/food/orders/{id}/messages`); непрочитанные считаются через `max_food_order_chat_reads`. При новом сообщении контрагенту уходит push в MAX (`LaravelOrderChatNotifier`).
...
│   ├── Contracts/
│   │   ├── Auth/                   # Gateway auth (DI)
│   │   ├── Food/                   # репозитории, notifier, dish image, order chat
│   │   └── Max/                    # initData validator, webhook router, order notification config
│   ├── DTO/                        # Food (Cart, Order, Admin*, OrderMessage), Max, Auth
...
│   │   │                           # AdminDish, AdminOrderReview
│   │   ├── Middleware/             # AuthenticateMaxMiniApp, EnsureFoodOrderAdmin,
│   │   │                           # TrustGatewayAuth, VerifyMaxWebhookSecret
│   │   ├── Requests/Food/          # cart, admin dishes, order chat, reject review
│   │   └── Responses/              # GatewayUnauthorizedResponse
│   ├── Models/                     # Restaurant, Dish, Cart, FoodOrder, FoodOrderMessage,
│   │                               # FoodOrderChatRead, MaxUser, CustomerCategory, …
│   ├── Repositories/Food/          # Eloquent* (dish, order, tier, category, admin, messages)
│   ├── Rules/                      # MinImageDimensions, ValidDishPhotoMime
│   ├── Services/
│   │   ├── Auth/                   # Gateway user resolver / session
│   │   ├── Food/                   # Cart*, Menu*, OrderSubmission*, DeliveryCost*,
│   │   │                           # Order*Review* (address/payment/composition),
│   │   │                           # OrderChat*, AdminOrderQuery*, CustomerOrderQuery*,
│   │   │                           # DishAdmin*, DishImage*, FoodOrderMaxMessageBuilder,
│   │   │                           # LaravelFoodOrderMaxNotifier, LaravelFoodOrderCustomerNotifier,
│   │   │                           # LaravelOrderChatNotifier, OrderStatusResolver
...
│   │   ├── api/foodClient.js
│   │   ├── bridge/maxBridge.js
│   │   ├── components/             # DishImage, OrderChat*, OrderStatusBadge, AppSelect, …
│   │   ├── composables/            # useAuth, useCart, useAdminFlow, useMyOrders, …
│   │   ├── constants/              # views.js, dishPhoto.js
│   │   ├── pages/                  # RestaurantList, Menu, Cart, OrderConfirmation,
│   │   │                           # OrderList, OrderDetail
│   │   └── pages/admin/            # AdminHome, AdminOrderList/Detail, AdminDishList/Form,
│   │                               # RejectOrderModal
...
└── tests/                          # Feature + Unit (БД: sail_db_testing)
    └── Support/                    # FoodTestDataBuilder, MaxInitDataFixtureBuilder, …
```
...
| `MAX_LOCAL_DEV_USER_ID` | Демо-пользователь: `1001` (Стандарт), `1002` (VIP), `1003` (админ адреса), `1004` (админ состава); профили — `config/max.php` → `local_dev_demo_users` |
...
| Food domain (unit) | `tests/Unit/CartServiceTest.php`, `CartTotalsCalculatorTest.php`, `DeliveryCostResolverTest.php`, `OrderStatusResolverTest.php` |
...
Вспомогательные фикстуры: `tests/Support/FoodTestDataBuilder.php`, `ResetsFoodDomainTables.php`, `AuthenticatesMaxMiniAppUser.php`, `MaxInitDataFixtureBuilder.php`, `DishPhotoTestImageFactory.php`, `MessMaxLogTestHelper.php`.
