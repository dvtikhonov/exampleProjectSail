...
function resolveMaxIdleIterations(targetCount: number, currentCount: number): number {
  ...
  const base = config.syncMaxIdleIterations;
  ...
  // ~1 «пустая» итерация на каждые 25 ещё не загруженных отзывов, минимум base.

  return Math.max(base, Math.ceil(remaining / 25));
...
export async function syncReviews(input: SyncReviewsInput): Promise<{ org: OrganizationMeta; reviews: ParsedReview[] }> {
  ...
  try {
    ...
    let reviews = unifyReviewsByContentPreferApiIds(dedupeReviews([
    ...
    const maxIdleIterations = resolveMaxIdleIterations(targetCount, reviews.length);
    ...
    while (idleIterations < maxIdleIterations) {
      ...
      reviews = unifyReviewsByContentPreferApiIds(dedupeReviews([...reviews, ...fromDom, ...fromPageState, ...fromNetwork]));
...
async function scrollReviewsPanel(page: Page): Promise<void> {
  ...
  const lastReview = page.locator('[class*="business-review-view"]').last();

  if ((await lastReview.count()) > 0) {

    await lastReview.scrollIntoViewIfNeeded().catch(() => {});
...
/** Кнопка «Показать ещё» на странице отзывов (если есть). */

async function tryLoadMoreReviews(page: Page): Promise<void> {

  const loadMore = page.locator('button, [role="button"], a').filter({

    hasText: /(?:ещё|еще|показать\s+ещ)/i,
  ...
  await loadMore.first().click({ timeout: 2000 }).catch(() => {});
